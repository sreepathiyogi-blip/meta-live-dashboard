...
class MetaAdsAPI:
    ...
    def fetch_historical_to_yesterday(self, account_id, start_date, level='campaign'):
        ...
        For ad-level data, breaks into monthly chunks to avoid API timeouts
        """
        yesterday = (datetime.now(Config.IST) - timedelta(days=1)).strftime('%Y-%m-%d')
        
        # For ad-level queries, break into monthly chunks to avoid 500 errors
        if level == 'ad':
            logger.info(f"  📅 Fetching HISTORICAL (monthly batches): {start_date} to {yesterday}")
            return self._fetch_in_monthly_batches(account_id, start_date, yesterday, level)
        else:
    ...
    def _fetch_in_monthly_batches(self, account_id, start_date, end_date, level):
        """
        Fetch data in monthly batches to avoid API errors with large date ranges
        """
        from dateutil.relativedelta import relativedelta
        ...
        while current_start <= end_dt:
            # Get end of current month or final end_date, whichever is earlier
            current_end = min(
            ...
            try:
                batch_data = self.fetch_custom_date_range(
                    account_id, batch_start, batch_end, level
                )
                all_data.extend(batch_data)
            except Exception as e:
                logger.warning(f"    ⚠️  Batch failed ({batch_start} to {batch_end}): {e}")
